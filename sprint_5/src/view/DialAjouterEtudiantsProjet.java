 ...
 * @version sprint3
 */
public class DialAjouterEtudiantsProjet extends JDialog implements ActionListener{
	...
	private DialListAjoutEtudiantProjet man;
	
	public DialAjouterEtudiantsProjet(DialListAjoutEtudiantProjet man, Etudiants etudiants){
		...
		ArrayList<String[]> temp = etudiants.getAllEtu();
		...
		if (event.getSource() == bOk){
			if (cEtudiant.getSelectedIndex() != 0){
				...
				if(etudiantCourant != null){
					etudiants.setGroupe(etudiantCourant[1], String.valueOf(this.idGroupe));		
				}
			}
			man.update();
			dispose();
